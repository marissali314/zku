...
describe('TornadoPool', function () {
  ...
  it("custom test", async function () {
    const { merkleTreeHistory, tornadoPool, token, omniBridge } = await loadFixture(fixture)
    const aliceKeypair = new Keypair()

    const gas = await merkleTreeHistory.estimateGas.insert(toFixedHex(123), toFixedHex(456))
    console.log('Leaf gas:', gas - 21000)
